...
import { HelpCircle, ImagePlus, Loader2, MoveRight, Plus } from "lucide-react";
...
const AddNewClipPostContent = () => {
  const { addClipPost, loading } = useAdminActions();
  ...
  async function onSubmit(values: z.infer<typeof formSchema>, e) {
    ...
    await addClipPost(values.title, values.description, videoID, imageFile);
  }
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex flex-row w-full items-center justify-between">
        <p className="font-[500] text-[1.4rem] pl-[0.5rem]">Добавить клип</p>
        <Link
          href="/clips"
          className={cn(
...
export default AddNewClipPostContent;
