...
const EditSongPageContent = ({ song, singleArtist }) => {
  const { updateSong, loading } = useAdminActions();

  //Is approved track or not
  const [isApproved, setIsApproved] = React.useState<boolean>(song.isApproved);

  //Artist doc
  const [artistDoc, setArtistDoc] = React.useState(singleArtist);
  ...
  //Track duration
  const [trackDuration, setTrackDuration] = React.useState(song.trackDuration);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>, e) {
    ...
    //Check if song file added and file lower than 20mb
    if (trackSongFile && trackSongFile > 20000000) {
      return toast({
        title: `Ошибка`,
        description: `Максимальный размер файла 20 мегабайт`,
        variant: "destructive2",
      ...
      <div className="w-full mt-[1.2rem] flex flex-row gap-[1.2rem]">
        ...
        <div className="flex flex-col w-full">
                ...
                <div className="flex flex-row w-full gap-[1.5rem]">
                  <div className="w-full max-md:w-full flex flex-col pointer-events-none select-none">
                    <div className="flex flex-row items-center max-md:justify-center">
                      ...
                      <div className="flex flex-col w-full">
                        <div
                          className="w-full h-[6rem] border-[2px] border-solid border-gray-200 items-center flex flex-row justify-center align-middle relative rounded-2xl overflow-hidden mr-[1.5rem] cursor-pointer transition-opacity hover:opacity-80  max-sm:h-[4rem] bg-cover bg-gray-50"
          ...
          <div className="w-full mt-[1.5rem]">
              <RotationSongsQuery
                whoDownloadIDandDateTime={song.whoDownloadIDandDateTime}
                songID={song.$id}
              />
          </div>
        </div>
...
export default EditSongPageContent;
