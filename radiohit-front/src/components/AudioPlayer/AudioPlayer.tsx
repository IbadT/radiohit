...
const AudioPlayer = ({ isMobile }) => {
  ...
  //Player size while sidebar hidden
  const needToBeWide =
    !isMobile && pathname.includes("/account") ? "!px-[22rem]" : "";

  //Player size on login | register pages
  // const needToBeRight =
  //     !isMobile && pathname.includes("/login" || "/register") ? "!pr-[2rem] !pl-[51vw]" : "";
  ...
  //Play next track
  const handleNextTrackPlay = async (songID) => {
    await getNextTrack(songID);
  };

  //Play prev track
  const handlePrevTrackPlay = async (songID) => {
    await getPrevTrack(songID);
  };
            ...
            <div className="flex flex-col p-[0.8rem] w-full mx-auto">
              ...
              <div className="grid grid-cols-3 items-center content-center justify-center max-md:flex max-md:flex-row max-md:justify-between">
                ...
                <div className="flex flex-row items-center justify-start">
                  ...
                  {/*Radio Animation*/}
                  {isRadioPlayed && (
                    <div className="relative h-[2.5rem] w-[2.5rem] bg-white border-[1px] border-mainAccent rounded-xl mr-[1rem] flex justify-center items-center max-md:bg-transparent">
                      {isRadioPlayed && !playing && (
                        <RadioTower
                          strokeWidth={1.2}
                          className={cn(
                            "h-[1.8rem] w-[1.8rem] transition-colors duration-300 text-fuchsia-700"
                          )}
                        />
                      )}
                      {isRadioPlayed && playing && (
                  ...
                  <div className="flex flex-col">
                    ...
                    {singleTrackData && (
                      <Link
                        scroll={false}
                        href={`/artists/${singleTrackData.trackArtistID}`}
                        className="text-[#ACACAD] text-[0.8rem] hover:underline duration-300 transition-all hover:text-mainAccent relative max-md:text-slate-600 pointer-events-none"
                      >
                    ...
                    {isRadioPlayed && (
                      <Link
                        ...
                        href={`/radio`}
                        className={cn(
                          "text-[#ACACAD] text-[0.8rem] hover:underline duration-300 transition-all hover:text-mainAccent relative max-md:text-slate-600",
                          radioStationSelected !== "radiohit" &&
                            "pointer-events-none"
                        )}
                      >
                ...
                <div className="flex flex-row items-center justify-center">
                  <PrevTrackIcon
                    className={cn(
                      "cursor-pointer transition-all duration-300 hover:scale-[1.05] hover:opacity-70 max-md:hidden",
                      isRadioPlayed && "opacity-30 pointer-events-none"
                    )}
                    onClick={() => handleNextTrackPlay(singleTrackData.$id)}
                  />
                  {/*Play / Pause*/}
                  <div
                  ...
                  <NextTrackIcon
                    ...
                    onClick={() => handlePrevTrackPlay(singleTrackData.$id)}
                  />
                </div>
