...
import { getRadioDownloadedTracks } from '@/lib/appwrite/db_services/songs.db.service';
import { ListMusic, Loader2 } from 'lucide-react';
import * as React from 'react';
import AccountAudioCard from '@/components/Cards/AudioCards/AccountAudioCard';

const AccountDownloadedTracksInner = ({ userDocument }) => {
  const { isLoading, isError, data, error, refetch } = useQuery({
    queryKey: ['radioDownLoadTracks'],
    queryFn: async () => {
      return await getRadioDownloadedTracks(userDocument.downloadTracksID);
    },
  });
...
export default AccountDownloadedTracksInner;
